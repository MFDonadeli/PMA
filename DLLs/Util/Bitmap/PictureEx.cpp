	...
	else
	{
		if (m_pPicture)
		{
			...
			if(m_bStretch)
			{
				...
				if (m_pPicture->Render(m_hMemDC, 0, 0, r.Width(), r.Height(), 
...
void CPictureEx::OnPaint() 
{
	...
	if (nPaintWidth > 0)
	{
		...
		{
			::StretchBlt(dc.m_hDC, 0, 0, nPaintWidth, nPaintHeight, m_hMemDC, m_PaintRect.left, m_PaintRect.top, 
				m_PaintRect.right-m_PaintRect.left, m_PaintRect.bottom-m_PaintRect.top, SRCCOPY);
		}
	}
	else
	{
...
BOOL CPictureEx::PrepareDC(int nWidth, int nHeight)
{
	...
	if(m_bStretch)
		m_hBitmap  = CreateCompatibleBitmap(hWinDC,r.Width(),r.Height());
	else
