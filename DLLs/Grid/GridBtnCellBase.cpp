...
#ifdef _WIN32_WCE
#if _WIN32_WCE == 0x420
int GridDrawTextEx(HDC hDC, LPCTSTR lpString, int nCount, LPRECT lpRect, UINT uFormat)
{
	//
    ...
    if( iCtlNbr > 0)
    {
                                ...
                                *prect) )     // cell rectangle to work with
        {
            for( int i1=0; i1 < iCtlNbr; i1++)
            {
                ...
                const TCHAR* pszBtnText = GetDrawCtlBtnText( i1);
                if( pszBtnText != NULL)
                {
...
#if _WIN32_WCE == 0x420
						GridDrawTextEx(pDC->GetSafeHdc(), pszBtnText, -1,
                                        RectAry[ i1],
        ...
        for( int i1=0; i1 < iCtlNbr; i1++)
        {
            ...
            if( iWidth <= 0)
            {
                ...
                else
                {
                    ...
                    pDC->DrawText( pszBtnText, wcslen( pszBtnText), &rectText, GetFormat() | DT_CALCRECT);
...
LPCTSTR CGridBtnCellBase::GetTipText()
{
    ...
    {
        // remove any '&' chars
        const TCHAR* pszCtlBtnText = GetDrawCtlBtnText( i1);
