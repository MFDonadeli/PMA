...
UINT CGridCtrl::WM_INSERT_LINE =
	RegisterWindowMessage(_T("CGridCtrl::WM_INSERT_LINE"));
...
LRESULT CGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage, CPoint point) const
{
    if (!IsWindow(m_hWnd))
        return 0;
...
BOOL CGridCtrl::SetClickedDivision(int nRow, int nCol, int nDivision)
{
	if (GetVirtualMode())
		return FALSE;
...
{
	CString s;
	s.Format(L"%S(%d): Row: %d Col: %d Text: %s **", __FUNCTION__, __LINE__, nRow, nCol, GetItemText(nRow, nCol));
	OutputDebugString(s);
	s.Format(L"GetItemState: %d **", GetItemState(nRow, nCol));
	OutputDebugString(s);
	s.Format(L"IsEditable: %d **", IsEditable());
	OutputDebugString(s);
	s.Format(L"IsEditableRow: %d **", IsEditableRow(nRow));
	OutputDebugString(s);
	s.Format(L"IsEditableCol: %d\r\n", IsEditableCol(nCol));
	OutputDebugString(s);

    if(IsEditable() && ((GetItemState(nRow, nCol) & GVIS_READONLY) != GVIS_READONLY))
		return TRUE;
	else if(IsEditableRow(nRow) && ((GetItemState(nRow, nCol) & GVIS_READONLY) != GVIS_READONLY))
		return TRUE;
	else if(IsEditableCol(nCol) && ((GetItemState(nRow, nCol) & GVIS_READONLY) != GVIS_READONLY))
		return TRUE;
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
...
#endif
    {
        ...
        else
        {
            ...
            m_nTimerID = (int)SetTimer(WM_LBUTTONDOWN, (UINT)m_nTimerInterval, 0);
        }
    }   
    m_LastMousePoint = point;
}
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    else
    {
        ...
        if (pCell)
            pCell->OnClick( GetPointClicked( m_idCurrentCell.row, m_idCurrentCell.col, point) );
        SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, NM_CLICK, point);
    }
...
void CGridCtrl::OnEditCell(int nRow, int nCol, CPoint point, UINT nChar)
{
	...
	if(nRow == GetRowCount()-1 && nCol == 0 && m_bInsertEditRow)
	{
		CWnd *pOwner = GetOwner();
		if (pOwner && IsWindow(pOwner->m_hWnd))
			pOwner->SendMessage(WM_INSERT_LINE, 0, 0);
	}
...
void CGridCtrl::OnSetFocus(CWnd *pOldWnd) 
{
	...
	SetFocusCell(1, 0);
	SetSelectedRange(1,0,1,m_nCols-1,TRUE);
}

void CGridCtrl::DefaultSetupGrid()
{
	...
	SetFixedBkColor(RGB(215,215,215));
