...
//#ifndef _WIN32_WCE
	...
	//CRect r = rect; //MMS
	//r.right = CheckRect.right; //MMS
	//pDC->FillSolidRect(r,RGB(255,0,0)); //MMS

    //rect.left = CheckRect.right;
	//rect.top  = CheckRect.top;
    ...
    // Do the draw 
#ifndef _WIN32_WCE
    pDC->DrawFrameControl(CheckRect, DFC_BUTTON, 
	    (m_bChecked)? DFCS_BUTTONCHECK | DFCS_CHECKED | DFCS_FLAT : DFCS_BUTTONCHECK | DFCS_FLAT);
#else
    pDC->DrawFrameControl(CheckRect, DFC_BUTTON, 
...
CRect CGridCellCheck::GetCheckPlacement()
{
	...
	int nWidth   = DRA::SCALEX(15);//GetSystemMetrics(SM_CXHSCROLL);
