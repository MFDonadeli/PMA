...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	if(GetDivisions() > 1)
	{
		int nDiv = rect.Width() / GetDivisions();
		CRect r;

		int nSize = 0;
		CStringArray sArr;

		CUtil::Tokenize(GetText(), sArr, nSize, L"**");
		
		for(int i=0; i<nSize; i++)
		{
			r = CRect(rect.left + 1 + (nDiv*i), rect.top, nDiv - 1 + (nDiv*i), rect.bottom);
			pDC->Draw3dRect(r , RGB(0,0,0), RGB(0,0,0));
			DrawText(pDC->m_hDC, sArr[i], -1, r, GetFormat() | DT_NOPREFIX | DT_CENTER);
		}
	}
	else
